...
ref_rows = subj_df[subj_df["rating_stimulus"].astype(str).str.strip().str.lower() == "reference"].copy()
ref_stats = (ref_rows.groupby("name")["rating_score"].agg(total_ref_trials="count",num_below_90=lambda s: (s < 90).sum()))

ref_stats["share_below_90"] = ref_stats["num_below_90"] / ref_stats["total_ref_trials"].clip(lower=1)
to_exclude = ref_stats[ref_stats["share_below_90"] > 0.15].index.tolist()
print("Vyřazení posluchači (hidden reference < 90 u >15 % případů):", to_exclude)
print(ref_stats.sort_values("share_below_90", ascending=False))

subj_df = subj_df[~subj_df["name"].isin(to_exclude)]

obj_speech_df = obj_speech_df.rename(columns={"degradation": "stimulus"})
obj_all_df = obj_all_df.rename(columns={"degradation": "stimulus"})
subj_df = subj_df.rename(columns={"rating_stimulus": "stimulus", "rating_score": "Score"})
...
def plot_degradation_comparison(df, degradation_key, metric, target):
    ...
    correlation, pvalue = spearmanr(subset[metric], subset["Score"])

    mean_df = subset.groupby(["name", "stimulus"])[[metric, "Score"]].mean().reset_index()

    mean_df["stimulus"] = pd.Categorical(mean_df["stimulus"], categories=degradation_types[degradation_key], ordered=True)

    plt.figure(figsize=(8, 4.5))
    sns.scatterplot(data=mean_df, x=metric, y="Score", hue="stimulus", palette="Set2", edgecolor="black", s=60)
    sns.regplot(data=mean_df, x=metric, y="Score", scatter=False, lowess=True, color="blue", line_kws={'linewidth': 2})

    plt.title(f"{degradation_key.capitalize()} – {metric} vs Poslechový test\nSpearman ρ = {correlation:.3f}, p-hodnota = {pvalue:.3f}")
    plt.xlabel(metric)
    plt.ylabel("Poslechové skóre")
    plt.grid(True)
...
def export_metric_table(df, metric, degradation_key, target):
    ...
    if metric not in subset.columns:
        return
    export_df = subset[["stimulus", "trial_id", metric, "Score"]].dropna().sort_values(by="stimulus")
